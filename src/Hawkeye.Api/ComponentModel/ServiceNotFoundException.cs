...
namespace Hawkeye.ComponentModel
{
    ...
    public class ServiceNotFoundException : Exception
    {
        ...
        public ServiceNotFoundException(Type type, Exception innerException) :
            base(BuildMessage(type), innerException)
        {
            _serviceType = type;
        }
        ...
        public ServiceNotFoundException(Type type, string message) :
            base(BuildMessage(type, message))
        {
            _serviceType = type;
        }
        ...
        public ServiceNotFoundException(Type type, string message, Exception innerException) :
            base(BuildMessage(type, message), innerException)
        {
            _serviceType = type;
        }
        ...
        public override void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            info.AddValue("ServiceType", _serviceType);
            base.GetObjectData(info, context);
        }

        private static string BuildMessage(Type type)
        {
            return BuildMessage(type, string.Empty);
        }

        private static string BuildMessage(Type type, string message)
        {
            return string.IsNullOrEmpty(message)
                ? string.Format(CultureInfo.CurrentCulture, DEFAULT_MESSAGE, type)
                : string.Format(CultureInfo.CurrentCulture, DEFAULT_MESSAGE2, type, message);
        }
    }
